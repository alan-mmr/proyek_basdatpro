...
@section('content')
  <h3>Master Satuan</h3>
  <p class="small-muted">Kelola satuan barang (Unit).</p>
  ...
  <div style="margin-bottom:15px; padding: 10px; background: #f9f9f9; border: 1px solid #eee; border-radius: 8px; display:flex; align-items: center; gap: 10px;">
    ...
    <a href="{{ route('satuan.index', ['status' => 'all']) }}" 
    ...
    <a href="{{ route('satuan.index', ['status' => '1']) }}" 
    ...
    <a href="{{ route('satuan.index', ['status' => '0']) }}" 
    ...
    <div style="margin-left:auto;">
        <a href="{{ route('satuan.create') }}" style="background:#007bff; color:white; padding:8px 15px; text-decoration:none; border-radius:4px; font-weight:bold;">
          + Tambah Satuan
        </a>
    </div>
  </div>
  ...
  <table>
    <thead>
      <tr style="background:#eee;">
        ...
        <th style="padding:10px;">Nama Satuan</th>
    ...
    <tbody>
      @forelse($rows as $r)
        <tr style="border-bottom:1px solid #eee;">
          <td style="padding:10px;">{{ $r->idsatuan }}</td>
          <td style="padding:10px;"><strong>{{ $r->nama_satuan }}</strong></td>
          ...
          <td style="padding:10px;">
            <a href="{{ route('satuan.edit', $r->idsatuan) }}" style="color:blue; text-decoration:underline;">Edit</a>
            
            @if(in_array(session('user.nama_role'), ['Super Admin','Superadmin','Admin']))
              <form method="POST" action="{{ route('satuan.destroy', $r->idsatuan) }}" style="display:inline; margin-left:5px;">
                 @csrf @method('DELETE')
                 <button type="submit" onclick="return confirm('Hapus permanen satuan ini?')" style="color:red; background:none; border:none; cursor:pointer; text-decoration:underline; padding:0;">Hapus</button>
              </form>
            @endif
          </td>
        </tr>
      @empty
        <tr><td colspan="4" style="padding:20px; text-align:center;">Tidak ada data.</td></tr>
      @endforelse
    </tbody>
  </table>
